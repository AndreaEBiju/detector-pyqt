...
There are **two different review features** with similar names — easy
to mix up. Pick based on what you're trying to do:

### 5a. Toolbar 🔍 Review — per-recording, live, interactive

For inspecting the model's predictions on **one currently-open
recording**. Lives on the main viewer's toolbar.

**How to access it:** the 🔍 Review button is **greyed out until
inference has produced predictions**. Sequence to enable it:

1. **File → Open** → pick a `.mat` or `.h5` recording
2. Click 🚀 **Run inference** on the toolbar
3. Wait ~30 seconds for predictions to compute
4. The 🔍 Review button becomes clickable
5. Click it → a dockable review panel slides in on the right

The panel walks you through the top-20 disagreements (predictions
outside your human-marked regions) one at a time:
...
- **💾 Save review JSON** writes
  `~/.detector/reviews/<recording_id>_review.json`.

### 5b. Phase 5 review HTMLs — batch, post-retrain, file-based

For evaluating **a newly-trained model's quality across every fold**
all at once. These are static HTML files on disk (not a UI feature),
generated by the retrain pipeline OR by the standalone `detector
review` CLI command.

**Where to find them after retrain:**
...
(One per recording-fold of LORO.)

**To view:** open any of the `.html` files in a browser. They're
self-contained — no internet, no Python needed. Drag-and-drop into
Chrome / Safari / Firefox works.

**To (re-)generate them for an existing model:**

```bash
python -m detector.cli review --model <path/to/model_v0.X.Y>
```

Parallelized across folds; auto-detects worker count. Adds `--workers
N` to override, `--fold <recording_id>` to do just one.

**To aggregate the judgments you click through:**

```bash
python -m detector.cli review --aggregate \
  --out-dir <artifacts_dir>/model_v0.X.Y/review/
```

Produces `review_summary.json` with `true_artifact_count`,
`borderline_count`, `false_positive_count` and rates.

### Which one should you use when?

| Task | Use |
|---|---|
| Labeling: refining bad regions on a recording you're actively working on | **5a** — interactive toolbar |
| Just-finished retrain: deciding whether v0.2.0 is better than v0.1.0 | **5b** — Phase 5 HTMLs |
| Spot-checking why the model flagged something on a single file | **5a** |
| Auditing a model's behavior across the entire training corpus | **5b** |
| Building a per-recording review JSON for shared labels with collaborators | **5a** |
